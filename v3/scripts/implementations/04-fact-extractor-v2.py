...
FACTS_JSON = Path("/Users/rohitvashist/.openclaw/workspace/v3/memory/facts.jsonl")
...
class FactExtractor:
    ...
    def _calculate_confidence(self, fact_type: str, match_text: str, context: str) -> float:
        ...
        confidence = 0.6  # Base confidence (raised for better metric capture)
        ...
        if 20 <= text_len <= 150:
            ...
            confidence += 0.1
        elif 3 <= text_len < 10:  # Short metrics like "50ms"
            confidence += 0.05
        ...
        # Signal 3: Clear sentence boundaries (skip for short metrics)
        if text_len >= 10 and match_text[0].isupper() and not match_text.endswith(('and', 'or', 'but')):
            confidence += 0.05
        
        # Signal 4: Fact type specific boosts
        if fact_type == 'metric':
            # Metrics get high confidence if they have numeric value + unit
    ...
    def _filter_false_positives(self, facts: List[Dict]) -> List[Dict]:
        ...
        for fact in facts:
            ...
            # Skip if too short or too long (metrics get special handling)
