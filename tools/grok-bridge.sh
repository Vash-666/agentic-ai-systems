...
# Sanitize user message for JSON (escape quotes, backslashes, newlines, control chars)
function sanitize_json() {
    ...
    str=$(echo "$str" | tr -d '\x00-\x08\x0B\x0C\x0E-\x1F')
    echo "$str"
}
...
function grok_health_check() {
    ...
    local test_key
    test_key=$(grep GROK_API_KEY "$ENV_FILE" | cut -d= -f2- | tr -d ' ' | tr -d '"' | tr -d "'")
    if [[ ! "$test_key" =~ ^xai- ]]; then
